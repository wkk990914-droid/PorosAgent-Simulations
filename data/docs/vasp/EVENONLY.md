...
EVENONLY=.TRUE. selects a subset of **k**-points with *C*1=*C*2=*C*3=1, and *n*1+*n*2+*n*3 even. It reduces the computational work load for HF type calculations by a factor two, but is only sensible for high symmetry cases (such as sc, fcc or bcc cells).
