...
ELPH\_TRANSPORT\_EMAX\_PLOT = [real]  
 Default: **ELPH\_TRANSPORT\_EMAX\_PLOT** = $\max(\varepsilon\_{n\mathbf{k}})$+5
...
By default, the upper energy limit is set to $\max(\varepsilon\_{n\mathbf{k}})$ + 5 eV, where $\varepsilon\_{n\mathbf{k}}$ are the electronic eigenvalues computed in the k-point mesh defined by the KPOINTS\_ELPH file.
